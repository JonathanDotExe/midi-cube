...
Scene PluginView::create(ViewHost &frame) {
	...
	if (this->play_metronome) {
		...
		metronome->property.bind(*this->play_metronome, plugin.get_lock());
	...
	if (this->metronome) {
		...
		bpm->property.bind_function<unsigned int>(std::bind(&Metronome::get_bpm, this->metronome), std::bind(&Metronome::set_bpm, this->metronome, std::placeholders::_1), plugin.get_lock());
	...
	if (this->master_volume) {
		...
		volume->property.bind(*master_volume, plugin.get_lock());
