...
public:
	...
	bool on_action(Control* control) {
		if (dynamic_cast<OrganSwitch*>(control)) {
			...
			BindableBooleanValue* val = dynamic_cast<BindableBooleanValue*> (cast->property);
			if (val) {
			...
			BindableTemplateValue<double>* val = dynamic_cast<BindableTemplateValue<double>*> (cast->property);
			if (val) {
			...
			BindableTemplateValue<int>* val = dynamic_cast<BindableTemplateValue<int>*> (cast->property);
			if (val) {
			...
			BindableTemplateValue<unsigned int>* val = dynamic_cast<BindableTemplateValue<unsigned int>*> (cast->property);
			if (val) {
